...
class PornHub:
  ...
  def GetContentLen(self):
      headers = {'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/73.0.3559.6 Safari/537.36'}
  ...
  def run(self):
      ...
      self.title = re.findall(r'\<title\>(.+?)\</title\>',str(content))[0].replace('/', '').replace('-','_').replace(' ','_').replace(';','_').replace(')','_').replace(':','_').replace('&','_').replace('#','_')
      print(self.title)
      c = json.loads(c)
      ...
      haveMore = False
      while True:
          future = p.submit(self.PHdownload, target[0], target[1],self.title)
